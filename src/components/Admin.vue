    ...
    <div v-if="pedidos.length==0" class="alert alert-warning mt-1">
      ...
      <table class="table" v-else>
          ...
          <tr class="bg-light" v-for="(p,index) in pedidos" :key="index">
              ...
              <td>{{ p.total }}</td>
              <td>{{ p.estado }}</td>
              <td><button class="btn btn-warning btn-sm" type="button" @click="anular(p)">Anular</button>
              <button class="btn btn-success btn-sm ml-1" type="button" @click="ver(p)">Ver</button></td>
          </tr>
      </table>
